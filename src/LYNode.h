...
public:
	...
	u16 sbusGetch(u8 ch){return sbus_frame[1+ch*2];}//解码sbus缓存返回通道数据
	u8 sbusStatus(){return sbus_frame[0];}//解码sbus缓存返回状态
	u8 subsGetNum(){return (sizeof(sbus_frame)-1)/2;}//subs通道数
private:
